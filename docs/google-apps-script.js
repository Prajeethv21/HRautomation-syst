...
// IMPORTANT: Values must EXACTLY match the tab names in Google Sheets.
// Sheet tab names as they currently exist in the spreadsheet:
//   AI/Data Engineer, Sustainability, Web Developer, Marketing, Creative, Others
const ROLE_TO_SHEET_MAP = {
  "Sustainability": "Sustainability",
  "AI Automation Engineer": "AI/Data Engineer",
...
function onEdit(e) {
  ...
  if (sheetName === "Candidates" && col === 5) {
    // Candidates status changed -> sync to Department sheet column J (10)
      ...
      if (deptSheet) {
        ...
        for (var i = 1; i < deptData.length; i++) {
          if (deptData[i][1] && deptData[i][1].toString().trim().toLowerCase() === email.toLowerCase()) {
            var deptStatusCell = deptSheet.getRange(i + 1, 10);
  ...
  } else if (sheetName !== "Candidates" && sheetName !== "ProcessedResumes" && sheetName !== "ProcessedResumesLog" && col === 10) {
...
function doPost(e) {
  ...
  try {
    ...
    var groqApiKey = data.groqApiKey || "";
    ...
    if (action === "processResumes") {
      return handleProcessResumes(sheetId, groqApiKey);
    }

    if (action === "repairData") {
      return handleRepairData(sheetId, groqApiKey);
    }
  ...
  try {
    ...
    if (deptSheetName) {
      ...
      if (deptSheet) {
        ...
        if (deptRowIndex !== -1) {
          // Status column in Department sheet is column 10 (J), not 11
          var deptStatusCell = deptSheet.getRange(deptRowIndex, 10);
...
// Helper: Build status dropdown validation rule
function createStatusValidationRule() {
...
// Helper: Apply status dropdown to a specific cell
function applyStatusDropdown(sheet, rowIndex, colIndex) {
  try {
    sheet.getRange(rowIndex, colIndex).setDataValidation(createStatusValidationRule());
  } catch (e) {
    Logger.log("[WARN] Could not apply status dropdown: " + e.toString());
  }
}
...
function handleCreateCandidate(sheetId, candidate) {
  try {
    ...
    var clean = {
      name: candidate.name ? candidate.name.toString().trim() : "",
      email: candidate.email ? candidate.email.toString().trim() : "",
      phoneNumber: candidate.phoneNumber ? candidate.phoneNumber.toString().trim() : "",
      ug: candidate.ug ? candidate.ug.toString().trim() : "",
      pg: candidate.pg ? candidate.pg.toString().trim() : "",
      college: candidate.college ? candidate.college.toString().trim() : "",
      location: candidate.location ? candidate.location.toString().trim() : "N/A",
      linkedin: candidate.linkedin ? candidate.linkedin.toString().trim() : "",
      github: candidate.github ? candidate.github.toString().trim() : "",
      role: candidate.role || "Sustainability",
    ...
    Logger.log("[CREATE] Candidate: " + clean.name + " | Role: " + clean.role);
    Logger.log("[CREATE] UG: " + clean.ug);
    Logger.log("[CREATE] PG: " + clean.pg);
    Logger.log("[CREATE] College: " + clean.college);
    Logger.log("[CREATE] Location: " + clean.location);
    Logger.log("[CREATE] LinkedIn: " + clean.linkedin);
    Logger.log("[CREATE] Phone raw value: " + JSON.stringify(clean.phoneNumber) + " (type: " + typeof clean.phoneNumber + ")");
    var phone = clean.phoneNumber;
    var phoneStr = String(phone || '');
    var phoneToWrite = "";
    var phoneStrLower = phoneStr.toLowerCase().trim();
    if (phoneStrLower && 
        phoneStrLower !== "undefined" && 
        phoneStrLower !== "null" && 
        phoneStrLower !== "object" && 
        phoneStrLower.indexOf("object") === -1 && 
        phoneStrLower.indexOf("#error") === -1) {
      phoneToWrite = formatSheetValue(phoneStr);
    }
    Logger.log("phoneToWrite: " + phoneToWrite);
    ...
    if (candidateExists) {
      Logger.log("[CREATE] SKIPPED: Duplicate email: " + clean.email);
    ...
    Logger.log("Candidate details before appendRow:\n" +
               "name: " + clean.name + "\n" +
               "ug: " + clean.ug + "\n" +
               "pg: " + clean.pg + "\n" +
    ...
    // Apply status dropdown to Candidates sheet (Status = col 5)
    var masterLastRow = masterSheet.getLastRow();
    applyStatusDropdown(masterSheet, masterLastRow, 5);
    Logger.log("[CREATE] Master sheet row appended at row " + masterLastRow);

    // 2. Add to Department sheet (exactly 10 columns)
    var deptSheetName = ROLE_TO_SHEET_MAP[clean.role] || clean.role;
    Logger.log("[CREATE] Routing to department sheet: " + deptSheetName);

    if (deptSheetName) {
      var deptSheet = getSheetByName(sheetId, deptSheetName);
      if (!deptSheet) {
        ...
        deptSheet.appendRow([
          "Candidate Name", "Email", "Phone Number", "UG", "PG",
          "College", "Location", "LinkedIn", "GitHub", "Status"
        ]);
        Logger.log("[CREATE] Created new department sheet: " + deptSheetName);
      }

      // Columns: Candidate Name, Email, Phone Number, UG, PG, College, Location, LinkedIn, GitHub, Status
      Logger.log("Candidate details before department appendRow:\n" +
                 "name: " + clean.name + "\n" +
                 "ug: " + clean.ug + "\n" +
                 "pg: " + clean.pg + "\n" +
                 "college: " + clean.college + "\n" +
      ...
      // Apply status dropdown to department sheet (Status = col 10)
      var deptLastRow = deptSheet.getLastRow();
      applyStatusDropdown(deptSheet, deptLastRow, 10);
...
function resolveRoleFolderId(parentId, roleName) {
  // Try exact name first
  var query = "title = '" + roleName.replace(/'/g, "\\'") + "' and mimeType = 'application/vnd.google-apps.folder' and trashed = false and '" + parentId + "' in parents";
  var list = Drive.Files.list({ q: query, maxResults: 1 });
  if (list.items && list.items.length > 0) {
    return list.items[0].id;
  }
  
  // If roleName contains a slash, try replacing with dash, space, or underscore
  if (roleName.indexOf('/') !== -1) {
    ...
    for (var i = 0; i < variations.length; i++) {
      var q = "title = '" + variations[i].replace(/'/g, "\\'") + "' and mimeType = 'application/vnd.google-apps.folder' and trashed = false and '" + parentId + "' in parents";
      var l = Drive.Files.list({ q: q, maxResults: 1 });
      if (l.items && l.items.length > 0) {
        Logger.log("[SCAN] Found existing matching folder for '" + roleName + "' named '" + variations[i] + "'");
        return l.items[0].id;
      }
    }
  }
  
  // If not found, create it using the original name
  var resource = {
    title: roleName,
  ...
  Logger.log("[SCAN] Created new folder for '" + roleName + "'");
  return newFolder.id;
}

// ============================================================
// ONE-TIME DATA REPAIR: Run this from the Apps Script editor
// to fix existing rows where college was merged into UG column.
// Instructions:
//   1. Open Apps Script editor
//   2. Select "repairDepartmentSheetData" in the function dropdown
//   3. Click the ▶ Run button
//   4. Open View > Logs to see what was fixed
// ============================================================
function repairDepartmentSheetData() {
  var SHEET_ID = "1KmEOk4qn0gF8pAbBUNCcrXuw2U4P3x18eVLAUxe1vtM";
  var ss = SpreadsheetApp.openById(SHEET_ID);

  // Skip these sheets — they are not department sheets
  var SKIP_SHEETS = ["Candidates", "ProcessedResumes", "ProcessedResumesLog"];

  // Institution name pattern — detect college names embedded in UG field
  var collegePattern = /\b(university|college|institute|school|academy|vidyapeeth|iit|nit|bits|zell|institution|deemed|center|centre|medical|technological|dayananda|ramaiah|sagar|jain|vtu|manipal|reva|rvce|bmsce|bms|pes|rnsit|msrit|christ|presidency)\b/i;

  var totalFixed = 0;

  // Auto-discover ALL sheets instead of hardcoding names
  var allSheets = ss.getSheets();
  Logger.log("[REPAIR] Found " + allSheets.length + " total sheets in spreadsheet.");
  ...
  for (var s = 0; s < allSheets.length; s++) {
    ...
    if (SKIP_SHEETS.indexOf(sheetName) !== -1) {
      Logger.log("[REPAIR] Skipping: " + sheetName);
      continue;
    }

    var data = sheet.getDataRange().getValues();
    Logger.log("[REPAIR] Scanning sheet: '" + sheetName + "' (" + (data.length - 1) + " data rows)");

    if (data.length <= 1) {
      Logger.log("[REPAIR] " + sheetName + " has no data rows, skipping.");
      continue;
    }

    // Log the header row to confirm column positions
    Logger.log("[REPAIR] Headers: " + JSON.stringify(data[0]));

    for (var i = 1; i < data.length; i++) {
      var ugVal = data[i][3] ? data[i][3].toString().trim() : "";     // col D = UG
      var pgVal = data[i][4] ? data[i][4].toString().trim() : "";     // col E = PG
      var collegeVal = data[i][5] ? data[i][5].toString().trim() : ""; // col F = College

      Logger.log("[REPAIR] Row " + (i+1) + " — UG: '" + ugVal + "' | PG: '" + pgVal + "' | College: '" + collegeVal + "'");

      if (!ugVal && !pgVal) continue;

      // Fix UG: if it contains a college-like word AND College column is empty
      var valToRepair = ugVal || pgVal;
      var colIndex = ugVal ? 3 : 4; // 0-based: col D or col E
      var sheetColIndex = ugVal ? 4 : 5; // 1-based for getRange: col 4 (D) or 5 (E)

      if (collegePattern.test(valToRepair) && !collegeVal) {
        var fixed = false;

        // Strategy 1: split on comma
        var commaParts = valToRepair.split(",");
        if (commaParts.length >= 2) {
          var degreeCandidate = commaParts[0].trim();
          var collegeCandidate = commaParts.slice(1).join(",").trim();
          if (collegePattern.test(collegeCandidate) && degreeCandidate.length > 3) {
            Logger.log("[REPAIR] ✓ Row " + (i+1) + " FIXED (comma split): '" + valToRepair + "' → degree='" + degreeCandidate + "' college='" + collegeCandidate + "'");
            sheet.getRange(i + 1, sheetColIndex).setValue(degreeCandidate);
            sheet.getRange(i + 1, 6).setValue(collegeCandidate); // College always col 6
            totalFixed++;
            fixed = true;
          }
        }

        // Strategy 2: split on dash / "at" / "from"
        if (!fixed) {
          var dashParts = valToRepair.split(/\s*[-–—]\s*|\s+at\s+|\s+from\s+/i);
          if (dashParts.length >= 2) {
            var degreeCandidate = dashParts[0].trim();
            var collegeCandidate = dashParts.slice(1).join(" - ").trim();
            if (collegePattern.test(collegeCandidate) && degreeCandidate.length > 3) {
              Logger.log("[REPAIR] ✓ Row " + (i+1) + " FIXED (dash split): '" + valToRepair + "' → degree='" + degreeCandidate + "' college='" + collegeCandidate + "'");
              sheet.getRange(i + 1, sheetColIndex).setValue(degreeCandidate);
              sheet.getRange(i + 1, 6).setValue(collegeCandidate);
              totalFixed++;
              fixed = true;
            }
          }
        ...
        if (!fixed) {
          Logger.log("[REPAIR] ✗ Row " + (i+1) + ": could not auto-split '" + valToRepair + "' — manual fix needed.");
        }
      }
    }
  }
  ...
  Logger.log("[REPAIR] ========================================");
  Logger.log("[REPAIR] Complete. Fixed " + totalFixed + " row(s) across all department sheets.");
  Logger.log("[REPAIR] ========================================");
  // NOTE: SpreadsheetApp.getUi() is intentionally NOT called here
  // because it causes errors when run from the Apps Script editor trigger.
}


function handleProcessResumes(sheetId, groqApiKey) {
  Logger.log("ENTERED handleProcessResumes");
  try {
    const count = processIncomingResumes(sheetId, groqApiKey);
...
function handleRepairData(sheetId, groqApiKey) {
  try {
    runExistingDataRepair(groqApiKey);
    return makeJsonResponse({ success: true, message: "Data repair and sync completed successfully." }, 200);
  } catch (error) {
    return makeJsonResponse({ success: false, message: "Data repair failed: " + error.toString() }, 500);
  }
}
...
function processNewResumes(sheetId, groqApiKey) {
  Logger.log("ENTERED processNewResumes");
  Logger.log("RESUME_FOLDER_ID = " + (typeof RESUME_FOLDER_ID !== "undefined" ? RESUME_FOLDER_ID : "undefined"));
  ...
  Logger.log("ROOT FOLDER ID: " + hrResumesFolderId);
  ...
  var totalCandidatesCreated = 0;

  var sources = ["LinkedIn", "Other"];
  // Scan ALL role folder names. Include both "AI/Data Engineer" and "AI Automation Engineer"
  // because Drive folders may have been created under either name.
  var roles = [
    "Sustainability",
    "AI/Data Engineer",
  ...
  sources.forEach(function(sourceName) {
    Logger.log("SOURCE FOLDER: " + sourceName);
    ...
    roles.forEach(function(roleName) {
      Logger.log("ROLE FOLDER: " + roleName);
      ...
      Logger.log("FILES FOUND: " + tempFiles.length);
      if (tempFiles.length === 0) {
        Logger.log("NO FILES FOUND IN ROLE FOLDER");
      }
      ...
      tempFiles.forEach(function(file) {
        ...
        try {
          ...
          if (extracted && extracted.text) {
            ...
            Logger.log("RAW PDF TEXT:\n" + rawText);
            
            var details = parseCandidateDetails(rawText, links, groqApiKey);
            details.resumeFileId = fileId;
            details.source = sourceName;
            // Normalize role: "AI Automation Engineer" folder → "AI/Data Engineer" role
            details.role = (roleName === "AI Automation Engineer") ? "AI/Data Engineer" : roleName;
            details.status = "Submitted";
            details.emailStatus = "Pending";
            Logger.log("[SCAN] Parsed fields — UG: '" + details.ug + "' | PG: '" + details.pg + "' | College: '" + details.college + "' | Role: '" + details.role + "'");
            
            if (!details.name) {
              throw new Error("Candidate name could not be extracted from resume.");
            }
  ...
  Logger.log(JSON.stringify({
    ...
    totalCandidatesCreated: totalCandidatesCreated
  }));
...
function processIncomingResumes(sheetId, groqApiKey) {
  Logger.log("ENTERED processIncomingResumes");
  return processNewResumes(sheetId, groqApiKey);
}
...
// LLM-based resume extraction using Groq API
function parseCandidateDetails(text, links, groqApiKey) {
  links = links || [];
  
  var apiKey = groqApiKey || PropertiesService.getScriptProperties().getProperty("GROQ_API_KEY");
  if (!apiKey) {
    throw new Error("GROQ_API_KEY is not configured (neither passed in request nor set in script properties).");
  }

  var linksText = "";
  if (links.length > 0) {
    linksText = "\n\nHyperlinks Extracted from Document Structure:\n" + 
      links.map(function(l) { return "- Link Text: \"" + l.text + "\", URL: \"" + l.url + "\""; }).join("\n");
  }

  var systemInstruction = "You are a precise resume parser. Your job is to extract candidate details from the full resume text and return ONLY valid JSON matching the exact requested schema.";

  var userPrompt = "Requirements:\n\n" +
  ...
  for (var m = 0; m < models.length; m++) {
    ...
    try {
      ...
      Logger.log("GROQ REQUEST SENT");
      
      var response = UrlFetchApp.fetch(url, options);
      responseCode = response.getResponseCode();
      responseText = response.getContentText();
      Logger.log("raw groq response: " + responseText);
      
      if (responseCode === 200) {
        Logger.log("GROQ RESPONSE RECEIVED");
        success = true;
        break;
      } else {
        Logger.log("[GROQ] Model " + modelName + " failed with code " + responseCode + ": " + responseText);
      }
    } catch (innerErr) {
      Logger.log("[GROQ] Fetch failed for model " + modelName + ": " + innerErr.toString());
    }
  }

  if (!success) {
    throw new Error("All Groq models failed or timed out. Last response status was " + responseCode + ": " + responseText);
  }
  ...
  Logger.log("parsedCandidate.phone = " + JSON.stringify(parsed.phone));
  Logger.log("parsedCandidate.phone: " + parsed.phone);
  
  var rawPhone = parsed.phone;
  var phoneStr = (rawPhone !== null && rawPhone !== undefined) ? String(rawPhone).trim() : "";
  Logger.log("phoneStr after String() conversion = " + JSON.stringify(phoneStr));
  
  var details = {
    name: (parsed.name || "").trim(),
    email: (parsed.email || "").trim(),
    phoneNumber: phoneStr,
    location: (parsed.location || "N/A").trim(),
 ...
 * After running, this function can be safely deleted or ignored.
 */
function runExistingDataRepair(groqApiKey) {
  var ss;
  try {
  ...
  const masterData = masterSheet.getDataRange().getValues();
  const masterHeaders = masterData[0].map(function(h) { return h.toString().trim().toLowerCase(); });
  
  var nameIdx = masterHeaders.indexOf("candidate name");
  var emailIdx = masterHeaders.indexOf("email address");
  if (emailIdx === -1) emailIdx = masterHeaders.indexOf("email");
  var roleIdx = masterHeaders.indexOf("role applied for");
  if (roleIdx === -1) roleIdx = masterHeaders.indexOf("role");
  ...
  for (var i = 1; i < masterData.length; i++) {
    ...
    var deptHeadersLower = deptHeaders.map(function(h) { return h.toString().trim().toLowerCase(); });
    var dPhoneIdx = deptHeadersLower.indexOf("phone number");
    if (dPhoneIdx === -1) dPhoneIdx = deptHeadersLower.indexOf("phone");
    var dUgIdx = deptHeadersLower.indexOf("ug");
    var dPgIdx = deptHeadersLower.indexOf("pg");
    var dCollegeIdx = deptHeadersLower.indexOf("college");
    var dLocIdx = deptHeadersLower.indexOf("location");
    var dLinkIdx = deptHeadersLower.indexOf("linkedin");
    var dGitIdx = deptHeadersLower.indexOf("github");
    ...
    if (fileId) {
      try {
        ...
        if (extracted && extracted.text) {
          console.log("RAW PDF TEXT:", extracted.text);
          Logger.log("RAW PDF TEXT:\n" + extracted.text);
          
          parsedDetails = parseCandidateDetails(extracted.text, extracted.links, groqApiKey);
    ...
    var clean = {
      name: (parsedDetails && parsedDetails.name) ? parsedDetails.name.toString().trim() : cName,
      email: email,
      phoneNumber: (parsedDetails && parsedDetails.phoneNumber) ? parsedDetails.phoneNumber.toString().trim() : (deptRowInfo && dPhoneIdx !== -1 ? deptRowInfo.rowValues[dPhoneIdx].toString().trim() : ""),
      ug: (parsedDetails && parsedDetails.ug) ? parsedDetails.ug.toString().trim() : (deptRowInfo && dUgIdx !== -1 ? deptRowInfo.rowValues[dUgIdx].toString().trim() : ""),
      pg: (parsedDetails && parsedDetails.pg) ? parsedDetails.pg.toString().trim() : (deptRowInfo && dPgIdx !== -1 ? deptRowInfo.rowValues[dPgIdx].toString().trim() : ""),
      college: (parsedDetails && parsedDetails.college) ? parsedDetails.college.toString().trim() : (deptRowInfo && dCollegeIdx !== -1 ? deptRowInfo.rowValues[dCollegeIdx].toString().trim() : ""),
      location: (parsedDetails && parsedDetails.location) ? parsedDetails.location.toString().trim() : (deptRowInfo && dLocIdx !== -1 ? deptRowInfo.rowValues[dLocIdx].toString().trim() : "N/A"),
      linkedin: (parsedDetails && parsedDetails.linkedin) ? parsedDetails.linkedin.toString().trim() : (deptRowInfo && dLinkIdx !== -1 ? deptRowInfo.rowValues[dLinkIdx].toString().trim() : ""),
      github: (parsedDetails && parsedDetails.github) ? parsedDetails.github.toString().trim() : (deptRowInfo && dGitIdx !== -1 ? deptRowInfo.rowValues[dGitIdx].toString().trim() : ""),
      role: role,
      status: status,
    ...
    console.log("SHEET ROW DATA:", rowData);
    Logger.log("SHEET ROW DATA:\n" + JSON.stringify(rowData, null, 2));
    ...
    if (deptSheet) {
      ...
      if (phoneStrLower && 
          ...
          phoneStrLower.indexOf("object") === -1 && 
          phoneStrLower.indexOf("#error") === -1) {
        repairPhoneToWrite = formatSheetValue(phoneStr);
      }
      Logger.log("phoneToWrite: " + repairPhoneToWrite);
      if (deptRowIndex === -1) {
        deptSheet.appendRow([
          clean.name,
          clean.email,
        ...
        deptSheet.getRange(deptRowIndex, 3).setValue(repairPhoneToWrite);
  ...
  // Restore validation dropdown rules
  for (var k = 2; k <= masterSheet.getLastRow(); k++) {
    applyStatusDropdown(masterSheet, k, statusIdx + 1);
  }
  
  var allSheets = ss.getSheets();
  var SKIP_SHEETS = ["Candidates", "ProcessedResumes", "ProcessedResumesLog"];
  for (var s = 0; s < allSheets.length; s++) {
    var dSheet = allSheets[s];
    var dName = dSheet.getName();
    if (SKIP_SHEETS.indexOf(dName) === -1) {
      for (var r = 2; r <= dSheet.getLastRow(); r++) {
        applyStatusDropdown(dSheet, r, 10);
      }
    }
  }
...
function verifyExtractionAccuracy() {
  ...
  for (var i = 1; i < masterData.length; i++) {
    ...
    var ugIsValid = !ugStr || (ugStr.length <= 150 && !/\b(responsibilities|worked|led|managed|implemented|experience|achievement|developed|built|designed|project|client|team|spearheaded|created|monitored)\b/i.test(ugStr));
    
    var pgStr = details.pg;
    var pgIsValid = !pgStr || (pgStr.length <= 150 && !/\b(responsibilities|worked|led|managed|implemented|experience|achievement|developed|built|designed|project|client|team|spearheaded|created|monitored)\b/i.test(pgStr));
...
// Helper to manually force/trigger OAuth permissions prompt for external HTTP requests
function triggerAuthorization() {
  Logger.log("Triggering explicit external request to force OAuth prompt...");
  try {
    var response = UrlFetchApp.fetch("https://api.groq.com", { muteHttpExceptions: true });
    Logger.log("Fetch succeeded! Status code: " + response.getResponseCode());
  } catch (e) {
...
function handleUploadResume(sheetId, fileData, fileName, roleName) {
  ...
  try {
    ...
    // Decode base64
    var fileBytes = Utilities.base64Decode(fileData);
    ...
    // Save to Google Drive in the role folder
    var folder = DriveApp.getFolderById(roleFolderId);
    var file = folder.createFile(blob);
    
    Logger.log("Successfully uploaded resume to Drive. File ID = " + file.getId() + " under role folder = " + roleName);
    return makeJsonResponse({ success: true, fileId: file.getId(), message: "Resume uploaded successfully to " + roleName + " folder." }, 200);
  } catch (error) {
